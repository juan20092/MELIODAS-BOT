...
const makeWALegacySocket = baileys.makeWALegacySocket || baileys.makeWASocket
const proto = baileys.proto || baileys.WAProto?.proto
const downloadContentFromMessage = baileys.downloadContentFromMessage
const jidDecode = baileys.jidDecode
const areJidsSameUser = baileys.areJidsSameUser
const generateWAMessage = baileys.generateWAMessage
const generateForwardMessageContent = baileys.generateForwardMessageContent
const generateWAMessageFromContent = baileys.generateWAMessageFromContent
const WAMessageStubType = baileys.WAMessageStubType
const extractMessageContent = baileys.extractMessageContent
    ...
    async value(jid, button, rows, quoted, options = {}) {
     ...
     if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
          ...
          forwardedNewsletterMessageInfo: {
            ...
            newsletterName: 'Rin Itoshi ¡New Channe! 🍂', 
            serverMessageId: '' }, 
                ...options
                }
                })
...
export function serialize() {
    ...
    if (!proto?.WebMessageInfo?.prototype) return null
